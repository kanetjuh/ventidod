module.exports = async function (channel, message, options) {
  const init = new Promise(async resolve => { // Create Promise
    ...
    // Assign Webhook
    let hook = webhooks.find(w => w.name === 'AutoHook')
    if (!hook) {
        try {
            hook = await channel.createWebhook('AutoHook', {
                avatar: 'https://gamemaster2030.github.io/Automator.png'
            });
        } catch (e) {
            hook = await channel.createWebhook('AutoHook', 'https://gamemaster2030.github.io/Automator.png');
        }
